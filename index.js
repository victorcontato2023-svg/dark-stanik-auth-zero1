...
/**
 * ✅ FALLBACK (evita 401 eterno quando o Render/GitHub tiver var com nome diferente)
 * Você pode manter só ADMIN_EMAIL/ADMIN_PASSWORD/JWT_SECRET no Render — funciona.
 */
const ADMIN_EMAIL =
...
app.get("/health", (req, res) => {
  res.json({
    ok: true,
    hasAdminEmail: !!String(ADMIN_EMAIL || "").trim(),
    hasAdminPassword: !!String(ADMIN_PASSWORD || "").trim(),
    hasJwtSecret: !!String(JWT_SECRET || "").trim(),
  });
});
...
app.post("/admin/login", (req, res) => {
  ...
  // se o env estiver vazio, avisa (evita ficar sem saber pq tá 401)
  if (!adminE || !adminP) {
    return res.status(500).json({
      success: false,
      error: "server_missing_admin_env",
    });
  }
...
app.post("/redeem", (req, res) => {
  ...
  if (!key || !deviceId) return res.status(400).json({ error: "missing data" });
  ...
  if (!found || found.used) return res.status(400).json({ error: "invalid code" });
...
app.post("/validate", (req, res) => {
  ...
  if (activations[k] === d) return res.json({ valid: true });
